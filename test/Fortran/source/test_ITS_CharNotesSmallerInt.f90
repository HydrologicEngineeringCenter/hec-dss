    SUBROUTINE test_ITS_CharNotesSmallerInt(ifltab1, messageUnit, status)
!
...
!
!
      WRITE(messageUnit, *)'Begin test_ITS_CharNotesSmallerInt'
      lowerCase = .false.
      nvals = 1000
...
!   First, write a single value and then read it back
 1  Format('Full Char Notes Irregular Smaller Interval Len test FAILED')   

    !  write 100 values and check
    cpart = 'Smaller Interval'
    ...
    !   a.  Merge, times on (i.e., Smaller interval)
    !   b.  Replace, times on (i.e., Smaller interval)
    !   c.  Merge, times off
    !   d.  Replace, times off
...
!
 ...
 810        Format('Full Irregular Interval Time Series, Smaller Interval Len Passed Successfully')  
        else
            Write(messageUnit, 820)
 820        Format('File Check FAILED in Full Irregular Smaller Interval Len') 
        endif         
       return
!
 900    Continue
