    SUBROUTINE test_ITS_CharNotesLargerInt(ifltab1, messageUnit, status)
!
...
!
!
      WRITE(messageUnit, *)'Begin test_ITS_CharNotesLargerInt'
      lowerCase = .false.
      nvals = 1000
...
!   First, write a single value and then read it back
 1  Format('Full Char Notes Irregular Larger Interval Len test FAILED')   

    !  write 100 values and check
    cpart = 'Larger Interval'
    ...
    !   a.  Merge, times on (i.e., Larger interval)
    !   b.  Replace, times on (i.e., Larger interval)
    !   c.  Merge, times off
    !   d.  Replace, times off
...
!            call zset('mlvl', '', 10)          
 ...
 220        continue
       ...
       call test_ITS_SupportCharNotes(ifltab1, messageUnit, status, storeFlag, &
            ...
            cpart, label, 100, 100, nread, ascending)
        ...
        call test_ITS_SupportCharNotes(ifltab1, messageUnit, status, storeFlag, &
            ...
            quality1, quality2, quality3, quality4, &
            cnotes1, cnotes2, cnotes3, cnotes4, &
            cpart, label, 300, 100, nread, ascending)
        if (status.ne.0) go to 900
...
!
 ...
 810        Format('Full Irregular Interval Time Series, Larger Interval Len Passed Successfully')  
        else
            Write(messageUnit, 820)
 820        Format('File Check FAILED in Full Irregular Larger Interval Len') 
        endif         
       return
!
 900    Continue
