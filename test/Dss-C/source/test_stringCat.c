...
int test_stringCat()
{
    ...
    status = stringCat(mess,sizeof(mess),proj,sizeof(proj));
    printf("\nstatus=%d len=%d",status,strlen(mess));
    // attempt to overflow
    status = stringCat(mess,sizeof(mess),"0123456789012345678901234567890123456789",sizeof(proj));
    printf("\nstatus=%d len=%d",status,strlen(mess));
    if( strlen(mess)> 499)
    {
