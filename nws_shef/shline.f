...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob5/rfc/ofs/src/shefpars_driv/RCS/shline.f,v $
     . $',                                                             '
...
C
  ...
  100     IF (LBEG.LE.LEND .OR. LSTAT.NE.0 .OR. LUNI.LT.0) GOTO 200
            ...
            IF (LSTAT .GT. 0) THEN
              ...
              ELSEIF (LSTAT .EQ. 0) THEN
                ...
                J = LBEG + 4
                IF (LINE(LBEG:J).EQ.':Run:') THEN
                   CALL CHRLNB(LINE(LBEG:), N)
                   CALL SHSRUNDA(LINE(LBEG:N))
                ENDIF
              ELSE
                KHPOS = 0
                KHAR  = ':'
  ...
  200     CONTINUE
     ...
     $      IOSTAT=LSTAT) LNUM
      ...
      RETURN
      END
      SUBROUTINE SHSRUNDA(LINE)
      CHARACTER*(*) LINE
      CHARACTER CDAT*12
C     Get run date from line:
C     :Run: 12/25/96  07:29 PST  
      CDAT = ' '
      I = INDEX(LINE, ' ')
      IF (I.LE.0) RETURN
      N = INDEX(LINE(I:), ' ')
      ...
      IF (LENG.GT.5) THEN
         CDAT = LINE(N:J)
         CALL DATYMD(CDAT, JYR, IMON, IDAY, IERR)
         IF (IERR.NE.0) RETURN
         CDAT = ' '
         WRITE (CDAT, '(I4.4)') JYR
         READ (CDAT, '(2I2)') ICENT, IYR
         CALL SHRUNDA('P', ICENT, IYR, IMON, IDAY, JSTAT)
      ENDIF
      RETURN
      END
