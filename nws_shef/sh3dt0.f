...
C                   If year or century num is missing, get from shyear
            ...
            ELSE
              CALL SHRUNDA('G', JCENT, JYR, JMON, JDAY, JSTAT)         
              IF (J3 .EQ. -1) THEN
                IMO = J1
                IDA = J2
                IF (JSTAT.EQ.0) THEN
...
C                   Check date for good numbers
      ...
      RETURN
      END
      SUBROUTINE SHRUNDA(CMD, JCENT, JYR, JMON, JDAY, JSTAT) 
C     Save and Get Run date     
      CHARACTER CMD*1
      INTEGER ICENT, IYR, IMON, IDAY
      INTEGER JCENT, JYR, JMON, JDAY, JSTAT
      SAVE ICENT, IYR, IMON, IDAY
      JSTAT = 0
      IF (CMD .EQ. 'I') THEN
            ICENT = 0
            IYR = 0
