      SUBROUTINE zrtext6 (IFLTAB, CPATH, IUNIT, IUHEAD, KUHEAD, NUHEAD,
     * LCCNTL, NLINES, ISTAT)
C
C
...
C
      ...
      CHARACTER CPATH*(*)
      INTEGER IUNIT, KUHEAD, NUHEAD, NLINES, ISTAT
      LOGICAL LCCNTL
C
      PARAMETER (KLINE=160)
      CHARACTER CLINE*(KLINE), CLINE2*(KLINE)
      LOGICAL LFOUND, LEND
C
...
C     Check that IFLTAB is valid (e.g., the DSS file is open)
      IF (IFLTAB(1).NE.6) CALL zerror6 (IFLTAB, 5, 'zrtext6',
     * 0, IFLTAB, ' ', 0, ' ',0)
C
C
C
...
C
 ...
 820  FORMAT(T5,'----- Exit zrtext6, Number of data values read:',I5,/,
...
C
 ...
 901  FORMAT (/,' *****DSS*** zrtext6:  ERROR - Record Not Identified',
...
C
 ...
 921  FORMAT (/,' *****DSS*** zrtext6:  ERROR During Write',
...
C
     ...
     * LCCNTL, NLINES, ISTAT)
C
C     FIX ME - This should be in the interface routines
C
      INTEGER IFLTAB(*), IUHEAD(*)
      CHARACTER CPATH*(*)
      INTEGER IUNIT, KUHEAD, NUHEAD, NLINES, ISTAT
...
C
      ...
      call zrtext6 (IFLTAB, CPATH, IUNIT, IUHEAD, KUHEAD, NUHEAD,
     * LCCNTL, NLINES, ISTAT)
      else
        call ztextRetrieveUnit(ifltab, cpath, IUNIT, NLINES, ISTAT)
      endif
      return
      end
