					   ...
					   int storageFlag, int boolReadBlock, int dataType)
{
	...
	if (boolReadBlock) {
		...
		if (recordFound == STATUS_RECORD_NOT_FOUND) {
			...
			if (boolReadBlock) {
				...
				int* adjustedValuesToStore = valuesToStore;

				// support saving floats to doubles
				// if the record type of this block is double, request doubles from existing block on disk
				if ( recordType == DATA_TYPE_ITD && valueElementSize == 1) {
						printf("\n'%s'\n", pathname);
						printf("floats:\n");
						float* f = (float*)valuesToStore;
					...
					valueElementSize = 2; // request doubles from disk
					valueSize = 2;
					dataType = DATA_TYPE_ITD;
					double* doublesToStore = calloc(numberToStore, sizeof(double));
					adjustedValuesToStore = (int*)doublesToStore;
					theTss = zstructTsClone(tss, pathname);
					free(theTss->floatValues);
					theTss->floatValues = NULL;
					theTss->allocated[zSTRUCT_TS_doubleValues] = 1;

					theTss->doubleValues = doublesToStore;

					if (doublesToStore) {
						convertDataArray(valuesToStore, adjustedValuesToStore, numberToStore, 1, 2);
						printf("doubles:\n");
						double* d = (double*)adjustedValuesToStore;
						for (double* fp = d; fp - d < numberToStore; ++fp) {
							printf("[%d]%2.f\n", (int)(fp - d), *fp);
						}
					}
					else {
				...
				printf("after ReadBlock --> doubles:\n");
				double* d = (double*)valuesRead;
				for (double* fp = d; fp - d < numberRead; ++fp) {
					printf("[%d]%2.f\n", (int)(fp - d), *fp);
				}
				...
				ztsIrregMergeBlocks(ifltab, storageFlag, tss, julianStartBlockDate,
									...
									adjustedValuesToStore, valuesRead, valueElementSize,
				...
				printf("valuesOut doubles:\n");
				d = (double*)valuesOut;
				for (double* fp = d; fp - d < totalToStore; ++fp) {
					printf("[%d]%2.f\n", (int)(fp - d), *fp);
				}
				printf("valuesOut floats:\n");
				float* f = (float*)valuesOut;
				for (float* fp = f; fp - f < totalToStore; ++fp) {
					printf("[%d]%2.f\n", (int)(fp - f), *fp);
				}
	...
	if (totalToStore > 0) {
		...
		status = ztsWriteBlock(ifltab, theTss, pathname,
		...
		printf("final write -good?  doubles:\n");
		double* d = (double*)values;
		for (double* fp = d; fp - d < totalToStore; ++fp) {
			printf("[%d]%2.f\n", (int)(fp - d), *fp);
		}
		printf("final write -good?  floats:\n");
