      SUBROUTINE zrrtsc7 (IFLTAB, CPATH, CDATE, CTIME, KVALS, NVALS,
...
C     This routine provides an alternative means from ZGTDTS of
...
C                 correct.  CPATH should be declared as CHARACTER*80
...
C                 record is to be retrieved (CDATE equal ' '), then NVALS
...
C                 successfulness of the retrieval.
C                 ISTAT = 0  All ok.
...
C
      IF (NUHEAD.GT.0) THEN
	   ...
	   NSUPP = MIN(NSUPP, KSUPP)
         CALL HOLCHR(IUHEAD, 1, NSUPP, CSUPP, 1)
      ENDIF
C
      RETURN
      END
