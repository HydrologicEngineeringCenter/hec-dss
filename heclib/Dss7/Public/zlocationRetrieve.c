...
void zlocationretrieve_(long long *ifltab, const char *path,
						...
						size_t pathLen, size_t timeZoneLen, size_t supplementalLen)
{
	...
	*status = zlocationRetrieve(ifltab, locationStruct);
	if (status != STATUS_RECORD_FOUND) {
		//  An error code
		zstructFree(locationStruct);
		return;
	...
	coordinateDescription[0] = locationStruct->coordinateSystem;
	coordinateDescription[1] = locationStruct->coordinateID;
	coordinateDescription[2] = locationStruct->horizontalUnits;
	coordinateDescription[3] = locationStruct->horizontalDatum;
	coordinateDescription[4] = locationStruct->verticalUnits;
	coordinateDescription[5] = locationStruct->verticalDatum;

	if (locationStruct->timeZoneName) {
		stringCToFort(timeZoneName, timeZoneLen, locationStruct->timeZoneName);
	}
	if (locationStruct->supplemental) {
		stringCToFort(supplemental, supplementalLen, locationStruct->supplemental);
	}
