...
JNIEXPORT void JNICALL Java_hec_heclib_util_Heclib_Hec_1zrrtsx
...
//	printf("Enter Heclib_Hec_zrrtsx\n ");
	...
	if (zmessageLevel((long long*)ifltab, MESS_METHOD_JNI_ID, MESS_LEVEL_INTERNAL_DIAG_1)) {		
		zmessageDebug((long long*)ifltab, DSS_FUNCTION_javaNativeInterface_ID, "Enter Heclib_Hec_zrrtsx, pathname: ", pathname);
	}
	zmessageDebug((long long*)ifltab, DSS_FUNCTION_javaNativeInterface_ID, "Enter Heclib_Hec_zrrtsx, pathname: ", pathname);
   zrrtsx_ ((long long*)ifltab, pathname, startDate, startTime, &number, data,
	...
	if (zmessageLevel((long long*)ifltab, MESS_METHOD_JNI_ID, MESS_LEVEL_USER_DIAG)) {
		zmessageDebugInt((long long*)ifltab, DSS_FUNCTION_javaNativeInterface_ID, "Exit Hec_zrrtsx; status: ", status[0]);
	}
