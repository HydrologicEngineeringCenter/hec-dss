...
JNIEXPORT void JNICALL Java_hec_heclib_util_Heclib_Hec_1zsitsx
...

{
	...
	if (zmessageLevel((long long*)ifltab, MESS_METHOD_JNI_ID, MESS_LEVEL_INTERNAL_DIAG_1)) {		
		zmessageDebug((long long*)ifltab, DSS_FUNCTION_javaNativeInterface_ID, "Enter Heclib_Hec_zsitsx, pathname: ", pathname);
	}

    zsitsx_ ((long long*)ifltab, pathname, times, values,
