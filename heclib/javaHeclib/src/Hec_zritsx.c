...
JNIEXPORT void JNICALL Java_hec_heclib_util_Heclib_Hec_1zritsx
     ...
     jint j_inflag, jintArray j_status)
{
    ...
    zritsx_ ((long long*)ifltab, pathname, &startJulian, &startMinutes,
	...
	if (zmessageLevel((long long*)ifltab, MESS_METHOD_JNI_ID, MESS_LEVEL_INTERNAL_DIAG_1)) {		
		zmessageDebugInt((long long*)ifltab, DSS_FUNCTION_javaNativeInterface_ID, "Exit Hec_zritsx, numberRead: ", numberRead[0]);
		zmessageDebugInt((long long*)ifltab, DSS_FUNCTION_javaNativeInterface_ID, "Hec_zritsx, startJulian: ", startJulian);
		zmessageDebugInt((long long*)ifltab, DSS_FUNCTION_javaNativeInterface_ID, "Hec_zritsx, endJulian: ", endJulian);
	}
